    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                # np.save('src_raw', pcd[:len_src].cpu().numpy())
                # np.save('tgt_raw', pcd[len_src:].cpu().numpy())
                # np.save('src_l_feats', l_feats[:len_src].cpu().numpy())
                # np.save('src_h_feats', h_feats[:len_src].cpu().numpy())
                # np.save('src_l_saliency', l_saliency[:len_src].cpu().numpy())
                # np.save('tgt_l_saliency', l_saliency[len_src:].cpu().numpy())
                # np.save('src_h_saliency', h_saliency[:len_src].cpu().numpy())
                # np.save('tgt_h_saliency', h_saliency[len_src:].cpu().numpy())
                # # np.save('src_overlap', scores_overlap[:len_src].cpu().numpy())
                # # np.save('tgt_overlap', scores_overlap[len_src:].cpu().numpy())
                # pdb.set_trace()
                ...
                feats = torch.cat((l_feats, h_feats), dim=-1)

                data = dict()
                data['pcd'] = pcd.cpu()
                data['feats'] = feats.detach().cpu()
                data['l_feats'] = l_feats.detach().cpu()
                data['h_feats'] = h_feats.detach().cpu()
                data['overlaps'] = scores_overlap.detach().cpu()
                data['l_saliency'] = l_saliency.detach().cpu()
                data['h_saliency'] = h_saliency.detach().cpu()
