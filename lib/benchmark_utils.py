...
def get_corr(src_pcd, tgt_pcd, est_rot, est_trans, thresh=0.1):
    src_pcd_t = (torch.matmul(to_tensor(est_rot.copy()), src_pcd.transpose(0, 1)) + to_tensor(est_trans.copy()).view(-1,1)).transpose(0, 1)
    query_list = torch.tensor([len(src_pcd_t)]).int()
    key_list = torch.tensor([len(tgt_pcd)]).int()
    neighbor = cpp_neighbors.batch_query(src_pcd_t.to(torch.device("cpu")), tgt_pcd.to(torch.device("cpu")), query_list,
                                         key_list, radius=thresh)
    # pdb.set_trace()
    M = tgt_pcd.shape[0]
    shadow_line = torch.ones_like(tgt_pcd[:1]) * 1e5
    tgt_pcd = torch.cat((tgt_pcd, shadow_line), 0)
    nearest_neighbor = neighbor[:,0]
    mask = (nearest_neighbor!= M)
    # if mask.sum() == 0:
    #     return None, None, 0
    src_pcd = src_pcd[mask]
    src_corr = tgt_pcd[nearest_neighbor[mask]]
    return src_pcd, src_corr, len(src_corr)

def get_neighbor(pcd, raw, radius=0.07):
    query_list = torch.tensor([len(pcd)]).int()
    key_list = torch.tensor([len(raw)]).int()
    neighbor = cpp_neighbors.batch_query(pcd.to(torch.device("cpu")), raw.to(torch.device("cpu")), query_list,
                                         key_list, radius=radius)
    # # pdb.set_trace()
    # neighbor_list = neighbor.reshape(-1)
    # neighbor_list = list(set(neighbor_list))
    M = raw.shape[0]
    # if M in neighbor_list:
    #     neighbor_list.remove(M)
    # raw = raw[neighbor_list]
    # feats = feats[neighbor_list]
    return torch.from_numpy(neighbor).long(), M
